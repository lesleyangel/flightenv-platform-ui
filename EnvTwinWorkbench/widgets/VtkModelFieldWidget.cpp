    ...
    : QWidget(parent)
{
    ...
    statusLabel_ = new QLabel(QStringLiteral("等待对象包 mesh layout 和真实场值"));
...
bool VtkModelFieldWidget::setMeshLayoutCatalog(const twin::PlatformMeshLayoutCatalog& catalog)
{
    meshCatalog_ = catalog;
    hasLoadedSurface_ = false;
    loadedLayoutId_.clear();
    vertexNodeIds_.clear();
    showMessage(QStringLiteral("mesh layout catalog 已接收: layouts=%1, source=%2")
                    .arg(catalog.layouts.size())
                    .arg(catalog.sourcePath));
    return !catalog.layouts.isEmpty();
}
    ...
    const QString& unit)
{
    ...
    if (!meshCatalog_.has_value()) {
        stats.message = QStringLiteral("缺少对象包 mesh layout catalog，无法把 artifact 场值映射到真实网格");
        clearField(stats.message);
        return stats;
    }

    QString error;
    if (!hasLoadedSurface_ || loadedLayoutId_ != layoutId) {
        if (!rebuildSurface(layoutId, &error)) {
            stats.message = error;
            clearField(stats.message);
            return stats;
        }
...
const twin::PlatformMeshLayoutView* VtkModelFieldWidget::findLayout(const QString& layoutId) const
{
    if (!meshCatalog_.has_value() || meshCatalog_->layouts.isEmpty()) {
        return nullptr;
    }
    const QString clean = layoutId.trimmed();
    const auto& layouts = meshCatalog_->layouts;
    auto it = std::find_if(layouts.begin(), layouts.end(), [&](const twin::PlatformMeshLayoutView& layout) {
        return layout.layoutId == clean;
    });
    if (it != layouts.end()) {
        return &(*it);
    }
...
VtkFieldRenderStats VtkModelFieldWidget::renderGeometryPreview(
    const QString& layoutId,
    const QString& title)
{
    VtkFieldRenderStats stats;
    if (!meshCatalog_.has_value()) {
        stats.message = QStringLiteral("缺少对象包 mesh layout catalog，无法显示真实对象网格");
        clearField(stats.message);
        return stats;
    }
    ...
    const auto* layout = findLayout(loadedLayoutId_);
    ...
    for (vtkIdType i = 0; i < static_cast<vtkIdType>(vertexNodeIds_.size()); ++i) {
        scalars_->SetValue(i, 0.5);
    }
    scalars_->Modified();
    surface_->GetPointData()->SetScalars(scalars_);
    surface_->Modified();

    lookup_->SetRange(0.0, 1.0);
    lookup_->SetTableRange(0.0, 1.0);
    lookup_->Build();
    mapper_->SetScalarRange(0.0, 1.0);
    mapper_->Modified();
    scalarBar_->SetTitle(title.toUtf8().constData());
    scalarBar_->SetLookupTable(lookup_);
    scalarBar_->Modified();
    renderWindow_->Render();
    ...
    stats.maxValue = 1.0;
    stats.meanValue = 0.5;
    stats.message = QStringLiteral("真实对象网格已显示: layout=%1, nodes=%2, vertices=%3")
                        .arg(loadedLayoutId_)
                        .arg(nodeCount)
...
bool VtkModelFieldWidget::rebuildSurface(const QString& layoutId, QString* error)
{
    const auto* layout = findLayout(layoutId);
    if (layout == nullptr) {
        if (error) {
            *error = QStringLiteral("缺少 layout=%1 的场布局").arg(layoutId);
        }
        return false;
    }
    if (layout->nodes.empty()) {
        if (error) {
            *error = QStringLiteral("layout=%1 的场布局没有节点坐标").arg(layout->layoutId);
        }
        return false;
    }

    const QString meshPath = layout->surfaceIndexPath;
    if (meshPath.isEmpty()) {
        if (error) {
            *error = QStringLiteral("缺少 layout=%1 的真实对象面片索引文件，不能绘制场云图").arg(layout->layoutId);
        }
        return false;
    }
    ...
    loadedLayoutId_ = layout->layoutId;
    hasLoadedSurface_ = true;
    return true;
}

bool VtkModelFieldWidget::loadIndexedSurface(const QString& path, const twin::PlatformMeshLayoutView& layout, QString* error)
{
    ...
    while (std::getline(file, line)) {
        ...
        const qsizetype idx1 = static_cast<qsizetype>(p1 - 1);
        const qsizetype idx2 = static_cast<qsizetype>(p2 - 1);
        const qsizetype idx3 = static_cast<qsizetype>(p3 - 1);
    ...
    const QString& unit)
{
    ...
    stats.nodeCount = nodeCount;
    if (nodeCount <= 0) {
        stats.message = QStringLiteral("mesh layout 节点数为 0");
        clearField(stats.message);
        return stats;
    }
    ...
    const auto expectedValueCount = static_cast<std::size_t>(nodeCount * dim);
    if (flattenedValues.size() != expectedValueCount) {
    ...
    stats.message = QStringLiteral("VTK 场已渲染: %1, layout=%2, nodes=%3, vertices=%4")
                        .arg(title, loadedLayoutId_)
                        .arg(nodeCount)
                        .arg(vertexNodeIds_.size());
    showMessage(stats.message);
    return stats;
